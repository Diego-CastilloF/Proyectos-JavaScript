function iniciarApp() {
  ...
  function mostrarRecetas(recetas = []) {
    ...
    heading.textContent = recetas.length ? `Recetas encontradas: ${recetas.length}` : 'No existen recetas';
  ...
  function mostrarReceta(receta) {
    ...
    for(let i = 1; i <= 20; i++) {
      if( receta[`strIngredient${i}`]  ) {
        ...
        ingredienteLi.textContent = `${ingrediente} ${signo} ${cantidad}`;
